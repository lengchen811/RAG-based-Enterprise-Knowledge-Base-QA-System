...
> 不只是跑通的 Demo，而是对齐生产级要求：**异步工程化、混合检索、重排序、全链路可观测、可容器化部署**。
...
### 1. 为什么用混合检索而不是纯向量？
纯向量检索对"专有名词"（如员工工号 `A1001`、产品型号 `X200`）极不敏感，语义相近但字面不同。BM25 精确匹配关键词，二者互补。实现上对双路结果做**线性加权融合**（也可用 RRF），以 `child_id` 去重取并集。

### 2. 父子文档策略解决了什么？
固定长度切分会把完整段落截断。这里先用 `MarkdownHeaderTextSplitter` 按标题切成语义完整的 **Parent**，再切成小块 **Child** 用于向量检索。检索命中 Child 后，取回其 Parent 喂给大模型——**检索时精准、生成时完整**。

### 3. 为什么引入异步任务？
上传百页 PDF 时，解析+向量化可能耗时数十秒，同步处理会撑爆 HTTP 超时。通过 Celery 异步化，接口秒回 `task_id`，前端轮询状态。数据库加状态机保证可追踪、可重试。

### 4. 重排序如何降低幻觉？
混合检索召回 Top-50 候选块，用 Cross-Encoder（`gte-rerank`）对"查询-块"逐对打分，只取 Top-5 送入大模型。相比双塔向量，Cross-Encoder 精度更高，显著减少无关上下文导致的幻觉。

### 5. 如何排查 Bad Case？
接入 LangSmith 记录每次请求的完整 Trace：检索召回哪些块、排序分数、Prompt 拼装、Token 消耗、模型延迟。据此判断是"检索没召回"还是"模型理解错"。
